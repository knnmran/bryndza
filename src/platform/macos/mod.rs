...
/// macOS platform implementation using Accessibility API
pub struct MacOSPlatform {
    config: Config,
    connection: Option<connection::MacOSConnection>,
}
...
impl Platform for MacOSPlatform {
    async fn connect(&mut self) -> Result<()> {
        let connection = connection::MacOSConnection::new(&self.config).await?;
    ...
    async fn screenshot(&self) -> Result<Vec<u8>> {
        let connection = self.connection.as_ref().ok_or_else(|| {
            crate::error::BryndzaError::session("Not connected to macOS platform")
        })?;
    ...
    async fn long_press_element(
        ...
        // Long press is not typically used on macOS desktop
